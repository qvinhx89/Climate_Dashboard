...
# ==================== CÀI ĐẶT TRANG ====================
st.set_page_config(page_title="Thiên Tai Toàn Cầu 2020-2025 | 24h Vàng & Mega-Event Paradox", layout="wide")
st.title("Phân tích Dữ liệu Thiên tai Toàn cầu 2020-2025")
st.markdown("**Đồ án cuối kỳ – Nhóm ...** | Dữ liệu: 3.000+ sự kiện thực tế")

# ==================== LOAD DATA ====================
@st.cache_data
def load_data():
    df = pd.read_csv('/content/drive/MyDrive/Research - UNKN Lab/Vinhx/Project1_UNKN/global_climate_events_economic_impact_2020_2025.csv')
...
df = load_data()
st.success(f"Dataset loaded thành công! Shape: {df.shape}")

# ==================== PHẦN 1: EDA CHI TIẾT (DỰA HOÀN TOÀN CODE CỦA BẠN) ====================
...
with tab1:
    st.subheader("Thông tin chung & Missing values")
    col1, col2 = st.columns(2)
    with col1:
        st.write("**df.info()**")
        buffer = df.info(buf=None)
        st.text(buffer if buffer else "Done")
    with col2:
        missing = df.isnull().sum()
        if missing.sum() == 0:
            st.success("Không có dữ liệu thiếu nào!")
        else:
    ...
    col2.metric("Số quốc gia", df['country'].nunique())
    col3.metric("Số loại thiên tai", df['event_type'].nunique())
    col4.metric("Thời gian", f"{df['date'].min().date()} → {df['date'].max().date()}")

with tab2:
    st.subheader("Phân phối các biến số chính (Log-transform)")
    cols_to_plot = ['economic_impact_million_usd', 'total_casualties', 'response_time_hours',
    ...
    for i, col in enumerate(cols_to_plot):
        ...
        axes[i].hist(log_col, bins=40, color='skyblue', edgecolor='black')
        axes[i].set_title(f'log({col})')
    plt.suptitle("Phân phối các biến số chính (Log-transform)", fontsize=16)
    plt.tight_layout()
    st.pyplot(fig)
    
    st.info("Dữ liệu bị lệch phải nghiêm trọng → cần binning/grouping thay vì dùng giá trị thô")

with tab3:
    st.subheader("Tần suất biến phân loại")
    col1, col2 = st.columns(2)
    with col1:
        fig, ax = plt.subplots(figsize=(8, 6))
        sns.countplot(y=df['event_type'], order=df['event_type'].value_counts().index[:10],
                      palette='viridis', ax=ax)
        ax.set_title('Top 10 loại thiên tai phổ biến nhất')
        st.pyplot(fig)
    with col2:
        fig, ax = plt.subplots(figsize=(8, 7))
        top15 = df['country'].value_counts().head(15)
        sns.countplot(y=df['country'], order=top15.index, palette='plasma', ax=ax)
        ax.set_title('Top 15 quốc gia có nhiều sự kiện nhất')
        st.pyplot(fig)

with tab4:
    st.subheader("Heatmap tương quan Pearson")
    key_cols = ['economic_impact_million_usd', 'total_casualties', 'deaths', 'injuries',
    ...
    st.warning("Tương quan tuyến tính rất yếu → cần phân tích sâu hơn theo nhóm (binning, clustering)")
...
col1, col2 = st.columns(2)
with col1:
    st.subheader("DQ1.1 – Tác động đến tử vong & thương tích")
    fig, ax = plt.subplots()
...
st.success("Insight: <24h giảm tử vong 48–52% → 24 giờ đầu là giới hạn vàng thực sự!")

st.subheader("DQ1.2 – Developing vs Developed")
col1, col2 = st.columns(2)
with col1:
    fig, ax = plt.subplots()
...
with col2:
    fig, ax = plt.subplots()
    sns.boxplot(data=df, x=df['country'].isin(['China','India']).map({True:'China + India', False:'Còn lại'}), 
...
st.balloons()
st.markdown("**Cảm ơn thầy đã theo dõi! Em sẵn sàng trả lời câu hỏi ạ**")
